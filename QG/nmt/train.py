...
def run_internal_eval(
  ...
  dev_eval_iterator_feed_dict = {
      ...
      eval_model.bin_file_placeholder:hparams.out_dir+'/'+'dev_binary.txt',
      eval_model.point_file_placeholder:hparams.out_dir+'/'+'dev_Point.txt',
  }
  ...
  if use_test_set and hparams.test_prefix:
    ...
    test_eval_iterator_feed_dict = {
        ...
        eval_model.bin_file_placeholder:hparams.out_dir+'/'+'test_binary.txt',
        eval_model.point_file_placeholder:hparams.out_dir+'/'+'test_Point.txt',
    }
...
def update_stats(stats, summary_writer, start_time, step_result):
  ...
  (_, step_loss, step_predict_count, step_summary, global_step,
   step_word_count, batch_size, grad_norm, learning_rate,kl_loss,value) = step_result
...
def check_stats(stats, global_step, steps_per_stats, hparams, log_f):
  ...
  utils.print_out(
      "  global step %d lr %g "
      "step-time %.2fs wps %.2fK ppl %.2f gN %.2f kl_loss %.2f kl_weight %0.2f %s" %
      (global_step, stats["learning_rate"],
       avg_step_time, speed, train_ppl, avg_grad_norm,stats['kl_loss'],stats['value'],
...
def train(hparams, scope=None, target_session=""):
  ...
  while global_step < num_train_steps:
    ### Run a step ###
    train_sess.run(tf.assign(train_model.model.get_global_step(),global_step))
    ...
    try:
      ...
      if hparams.decay_scheme=="patience" and dev_scores<hparams.best_bleu:
        if hparams.patience==0:
            break
        else:
            hparams.patience-=1
            hparams.learning_rate/=2
            train_sess.run(tf.assign(train_model.lrate(),hparams.learning_rate))
      continue
...
def _sample_decode(model, global_step, sess, hparams, iterator, src_data,
  ...
  for i in range(2):
      ...
      nmt_outputs, attention_summary,nmt_ids = model.decode(sess)
...
def _external_eval(model, global_step, sess, hparams, iterator,
                   iterator_feed_dict, tgt_file,src_file, label, summary_writer,
                   save_on_best):
