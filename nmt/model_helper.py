...
def create_point(hparams):
  src_file = "%s.%s" % (hparams.train_prefix, hparams.src)
  tgt_file = "%s.%s" % (hparams.train_prefix, hparams.tgt)
  src=open(src_file,'r')
  tag=open(tgt_file,'r')
    
  Point=open(hparams.out_dir+'/'+'train_Point.txt','w')
  binary=open(hparams.out_dir+'/'+'train_binary.txt','w')
  for i,j in zip(src,tag):
    source=i.strip().split()
    target=j.strip().split()
    b=[]
    p=[]
    for word in target:
        if word in source:
            p.append(str(source.index(word)))
            b.append('1')
        else:
            p.append('-1')
            b.append('0')
  ...
  src_file = "%s.%s" % (hparams.dev_prefix, hparams.src)
  tgt_file = "%s.%s" % (hparams.dev_prefix, hparams.tgt)
  src=open(src_file,'r')
  tag=open(tgt_file,'r')
  Point=open(hparams.out_dir+'/'+'dev_Point.txt','w')
  binary=open(hparams.out_dir+'/'+'dev_binary.txt','w')
  for i,j in zip(src,tag):
    source=i.strip().split()
    target=j.strip().split()
    b=[]
  ...
  src=open(src_file,'r')
  tag=open(tgt_file,'r')
  Point=open(hparams.out_dir+'/'+'test_Point.txt','w')
  binary=open(hparams.out_dir+'/'+'test_binary.txt','w')
  for i,j in zip(src,tag):
    source=i.strip().split()
    target=j.strip().split()
    b=[]
...
def create_train_model(
  ...
  graph = tf.Graph()
  bin_file=hparams.out_dir+'/'+'train_binary.txt'
  point_file=hparams.out_dir+'/'+'train_Point.txt'
  with graph.as_default(), tf.container(scope or "train"):
    ...
    bin_dataset = tf.data.TextLineDataset(bin_file)
    point_dataset = tf.data.TextLineDataset(point_file)
...
class EvalModel(
    collections.namedtuple("EvalModel",
                           ("graph", "model", "src_file_placeholder",
                            "tgt_file_placeholder","bin_file_placeholder","point_file_placeholder", "iterator"))):
  pass
...
def create_eval_model(model_creator, hparams, scope=None, extra_args=None):
  ...
  with graph.as_default(), tf.container(scope or "eval"):
    ...
    bin_file_placeholder = tf.placeholder(shape=(), dtype=tf.string)
    point_file_placeholder = tf.placeholder(shape=(), dtype=tf.string)
    src_dataset = tf.data.TextLineDataset(src_file_placeholder)
    tgt_dataset = tf.data.TextLineDataset(tgt_file_placeholder)
    bin_dataset = tf.data.TextLineDataset(bin_file_placeholder)
    point_dataset = tf.data.TextLineDataset(point_file_placeholder)
    iterator = iterator_utils.get_iterator(
        bin_dataset,
        point_dataset,
        src_dataset,
  ...
  return EvalModel(
      ...
      bin_file_placeholder=bin_file_placeholder,
      point_file_placeholder=point_file_placeholder,
      iterator=iterator)
...
def _create_pretrained_emb_from_txt(
  ...
  emb_dict, emb_size = vocab_utils.load_embed_txt(embed_file,vocab_file)
